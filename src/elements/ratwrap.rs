/*
use {
    crate::{
        element::ReceivableEventChanges, Context, DrawChPos, DynLocationSet, DynVal, Element,
        ElementID, Event, EventResponses, Pane, Priority, SortingHat, UpwardPropagator, ZIndex,
    },
    ratatui::{
        buffer::Buffer,
        layout::Rect,
        widgets::{StatefulWidget, Widget},
    },
    std::{cell::RefCell, rc::Rc},
};

// TODO ascii of a rat in a burrito

pub struct RatWrap {
    pub pane: Pane,
    pub widg: Rc<RefCell<dyn SizedWidget>>,
}

trait SizedWidget: Widget + Sized {}

impl RatWrap {
    pub fn new(hat: &SortingHat, widg: Rc<RefCell<dyn SizedWidget>>) -> Self {
        Self {
            pane: Pane::new(hat, "debug_size_pane"),
            widg,
        }
    }

    pub fn with_height(self, h: DynVal) -> Self {
        self.pane.set_dyn_height(h);
        self
    }

    pub fn with_width(self, w: DynVal) -> Self {
        self.pane.set_dyn_width(w);
        self
    }

    pub fn with_z(self, z: ZIndex) -> Self {
        self.pane.set_z(z);
        self
    }
}

impl Element for RatWrap {
    fn kind(&self) -> &'static str {
        self.pane.kind()
    }
    fn id(&self) -> ElementID {
        self.pane.id()
    }
    fn receivable(&self) -> Vec<(Event, Priority)> {
        self.pane.receivable()
    }
    fn receive_event_inner(&self, ctx: &Context, ev: Event) -> (bool, EventResponses) {
        self.pane.receive_event(ctx, ev.clone())
    }
    fn change_priority(&self, ctx: &Context, p: Priority) -> ReceivableEventChanges {
        self.pane.change_priority(ctx, p)
    }
    fn drawing(&self, _ctx: &Context) -> Vec<DrawChPos> {
        //let size = ctx.s;
        ////let backend = TestBackend::new(80, 30);
        ////let Ok(mut terminal) = Terminal::new(backend) else {
        ////    return vec![];
        ////};

        //// Create the Protocol which will be used by the widget.
        //let image = picker.new_resize_protocol(dyn_img);

        //f.render_stateful_widget(image, f.size(), &mut app.image);

        let area = Rect::new(0, 0, 80, 30);
        let mut buf = Buffer::empty(area);
        self.widg.borrow().render(area, &mut buf);
        vec![]
    }

    fn get_attribute(&self, key: &str) -> Option<Vec<u8>> {
        self.pane.get_attribute(key)
    }
    fn set_attribute(&self, key: &str, value: Vec<u8>) {
        self.pane.set_attribute(key, value)
    }
    fn set_upward_propagator(&self, up: Box<dyn UpwardPropagator>) {
        self.pane.set_upward_propagator(up)
    }
    fn set_hook(&self, kind: &str, el_id: ElementID, hook: Box<dyn FnMut(&str, Box<dyn Element>)>) {
        self.pane.set_hook(kind, el_id, hook)
    }
    fn remove_hook(&self, kind: &str, el_id: ElementID) {
        self.pane.remove_hook(kind, el_id)
    }
    fn clear_hooks_by_id(&self, el_id: ElementID) {
        self.pane.clear_hooks_by_id(el_id)
    }
    fn call_hooks_of_kind(&self, kind: &str) {
        self.pane.call_hooks_of_kind(kind)
    }
    fn get_dyn_location_set(&self) -> Rc<RefCell<DynLocationSet>> {
        self.pane.get_dyn_location_set()
    }
    fn get_visible(&self) -> Rc<RefCell<bool>> {
        self.pane.get_visible()
    }
}

//fn render(self, area: Rect, buf: &mut Buffer)

// ----------------
// displays the size
#[derive(Clone)]
pub struct StatefulRatWrap<STATE> {
    pub pane: Pane,
    pub widg: Rc<RefCell<dyn StatefulWidget<State = STATE>>>,
    pub state: Rc<RefCell<STATE>>,
}

impl<STATE> StatefulRatWrap<STATE> {
    pub fn new(
        hat: &SortingHat, widg: Rc<RefCell<dyn StatefulWidget<State = STATE>>>,
        state: Rc<RefCell<STATE>>,
    ) -> Self {
        Self {
            pane: Pane::new(hat, "debug_size_pane"),
            widg,
            state,
        }
    }

    pub fn with_height(self, h: DynVal) -> Self {
        self.pane.set_dyn_height(h);
        self
    }

    pub fn with_width(self, w: DynVal) -> Self {
        self.pane.set_dyn_width(w);
        self
    }

    pub fn with_z(self, z: ZIndex) -> Self {
        self.pane.set_z(z);
        self
    }
}

/*
impl<STATE> Element for StatefulRatWrap<STATE> {
    fn kind(&self) -> &'static str {
        self.pane.kind()
    }
    fn id(&self) -> ElementID {
        self.pane.id()
    }
    fn receivable(&self) -> Vec<(Event, Priority)> {
        self.pane.receivable()
    }
    fn receive_event_inner(&self, ctx: &Context, ev: Event) -> (bool, EventResponses) {
        self.pane.receive_event(ctx, ev.clone())
    }
    fn change_priority(&self, ctx: &Context, p: Priority) -> ReceivableEventChanges {
        self.pane.change_priority(ctx, p)
    }
    fn drawing(&self, _ctx: &Context) -> Vec<DrawChPos> {
        //let size = ctx.s;
        ////let backend = TestBackend::new(80, 30);
        ////let Ok(mut terminal) = Terminal::new(backend) else {
        ////    return vec![];
        ////};

        //// Create the Protocol which will be used by the widget.
        //let image = picker.new_resize_protocol(dyn_img);

        //f.render_stateful_widget(image, f.size(), &mut app.image);

        let area = Rect::new(0, 0, 80, 30);
        let buf = String::new();
        self.widg
            .borrow_
            .render(self, area, buf, self.state.borrow());
        vec![]
    }

    fn get_attribute(&self, key: &str) -> Option<Vec<u8>> {
        self.pane.get_attribute(key)
    }
    fn set_attribute(&self, key: &str, value: Vec<u8>) {
        self.pane.set_attribute(key, value)
    }
    fn set_upward_propagator(&self, up: Box<dyn UpwardPropagator>) {
        self.pane.set_upward_propagator(up)
    }
    fn set_hook(&self, kind: &str, el_id: ElementID, hook: Box<dyn FnMut(&str, Box<dyn Element>)>) {
        self.pane.set_hook(kind, el_id, hook)
    }
    fn remove_hook(&self, kind: &str, el_id: ElementID) {
        self.pane.remove_hook(kind, el_id)
    }
    fn clear_hooks_by_id(&self, el_id: ElementID) {
        self.pane.clear_hooks_by_id(el_id)
    }
    fn call_hooks_of_kind(&self, kind: &str) {
        self.pane.call_hooks_of_kind(kind)
    }
    fn get_dyn_location_set(&self) -> Rc<RefCell<DynLocationSet>> {
        self.pane.get_dyn_location_set()
    }
    fn get_visible(&self) -> Rc<RefCell<bool>> {
        self.pane.get_visible()
    }
}
*/
*/
