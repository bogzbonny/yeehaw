...
impl FixtureCell {
    #[allow(dead_code)]
    pub fn from_cell(cell: &vt100_yh::Cell) -> Self {
        Self {
...
pub struct FixtureScreen {
    ...
    mouse_protocol_mode: vt100_yh::MouseProtocolMode,
    #[serde(
        default,
    ...
    mouse_protocol_encoding: vt100_yh::MouseProtocolEncoding,
}
...
impl FixtureScreen {
    ...
    #[allow(dead_code)]
    pub fn from_screen(screen: &vt100_yh::Screen) -> Self {
        let empty_screen = vt100_yh::Parser::default().screen().clone();
...
fn deserialize_color<'a, D>(
    deserializer: D,
) -> std::result::Result<vt100_yh::Color, D::Error>
where
    D: serde::de::Deserializer<'a>,
{
    let val = <Option<String>>::deserialize(deserializer)?;
    match val {
        None => Ok(vt100_yh::Color::Default),
        Some(x) if x.starts_with('#') => {
            ...
            Ok(vt100_yh::Color::Rgb(r, g, b))
        }
        Some(x) => Ok(vt100_yh::Color::Idx(
...
{
    let s = match color {
        vt100_yh::Color::Default => unreachable!(),
        vt100_yh::Color::Idx(n) => format!("{n}"),
        vt100_yh::Color::Rgb(r, g, b) => format!("#{r:02x}{g:02x}{b:02x}"),
    };
...
fn deserialize_mouse_protocol_mode<'a, D>(
    deserializer: D,
) -> std::result::Result<vt100_yh::MouseProtocolMode, D::Error>
where
    D: serde::de::Deserializer<'a>,
{
    let name = <String>::deserialize(deserializer)?;
    match name.as_ref() {
        "none" => Ok(vt100_yh::MouseProtocolMode::None),
        "press" => Ok(vt100_yh::MouseProtocolMode::Press),
        "press_release" => Ok(vt100_yh::MouseProtocolMode::PressRelease),
        "button_motion" => Ok(vt100_yh::MouseProtocolMode::ButtonMotion),
        "any_motion" => Ok(vt100_yh::MouseProtocolMode::AnyMotion),
        _ => unimplemented!(),
    }
}

fn serialize_mouse_protocol_mode<S>(
    mode: &vt100_yh::MouseProtocolMode,
    serializer: S,
) -> Result<S::Ok, S::Error>
where
    S: serde::Serializer,
{
    let s = match mode {
        vt100_yh::MouseProtocolMode::None => "none",
        vt100_yh::MouseProtocolMode::Press => "press",
        vt100_yh::MouseProtocolMode::PressRelease => "press_release",
        vt100_yh::MouseProtocolMode::ButtonMotion => "button_motion",
        vt100_yh::MouseProtocolMode::AnyMotion => "any_motion",
    };
    serializer.serialize_str(s)
}
...
fn deserialize_mouse_protocol_encoding<'a, D>(
    deserializer: D,
) -> std::result::Result<vt100_yh::MouseProtocolEncoding, D::Error>
where
    D: serde::de::Deserializer<'a>,
{
    let name = <String>::deserialize(deserializer)?;
    match name.as_ref() {
        "default" => Ok(vt100_yh::MouseProtocolEncoding::Default),
        "utf8" => Ok(vt100_yh::MouseProtocolEncoding::Utf8),
        "sgr" => Ok(vt100_yh::MouseProtocolEncoding::Sgr),
        _ => unimplemented!(),
    }
}

fn serialize_mouse_protocol_encoding<S>(
    encoding: &vt100_yh::MouseProtocolEncoding,
    serializer: S,
) -> Result<S::Ok, S::Error>
where
    S: serde::Serializer,
{
    let s = match encoding {
        vt100_yh::MouseProtocolEncoding::Default => "default",
        vt100_yh::MouseProtocolEncoding::Utf8 => "utf8",
        vt100_yh::MouseProtocolEncoding::Sgr => "sgr",
    };
    serializer.serialize_str(s)
}
